...
{	int j = 0, h = now._nr_pr;
	...
	case 0:	/* mutex */
		...
		/* params: */
		/* locals: */
		((P0 *)pptr(h))->_1_1_other = 0;
#ifdef VAR_RANGES
		logval("mutex:other", ((P0 *)pptr(h))->_1_1_other);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
iniglobals(int calling_pid)
{
	...
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			logval("interested[l_in]", now.interested[l_in]);
		}
	}
		logval("cnt", now.cnt);
#endif
...
{	/* int i; */
	...
	{	int l_in;
		for (l_in = 0; l_in < 2; l_in++)
		{
			printf("	int    interested[%d]:	%d\n", l_in, now.interested[l_in]);
		}
	}
	printf("	int    cnt:	%d\n", now.cnt);
}
...
{	/* int i; */
	switch(tp) {
	case 0:
		printf("local vars proc %d (mutex):\n", pid);
	printf("	int    other:	%d\n", ((P0 *)pptr(pid))->_1_1_other);
		break;
	}
}
void
